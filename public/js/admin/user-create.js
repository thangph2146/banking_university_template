/**
 * Quản lý trang tạo người dùng mới
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Xử lý checkbox tạo mật khẩu tự động
  ...
  // Xử lý tải dữ liệu người dùng nếu là chế độ cập nhật
  initUserDataLoad();
});
 ...
 * Xử lý checkbox tạo mật khẩu tự động
 */
const initAutoPasswordGeneration = () => {
  ...
  if (!autoGenerateCheckbox || !passwordField || !confirmPasswordField) return;
  
  autoGenerateCheckbox.addEventListener('change', () => {
    if (autoGenerateCheckbox.checked) {
      // Nếu checkbox được chọn, vô hiệu hóa các trường mật khẩu
      passwordField.disabled = true;
      confirmPasswordField.disabled = true;
      passwordField.value = '';
      confirmPasswordField.value = '';
    } else {
      // Nếu checkbox bỏ chọn, kích hoạt lại các trường mật khẩu
...
const initFormHandling = () => {
  ...
  userForm.addEventListener('submit', (e) => {
    ...
    // Kiểm tra mật khẩu (chỉ khi không tự động tạo)
    const autoGenerateCheckbox = document.getElementById('auto_generate_password');
    const passwordField = document.getElementById('mat_khau');
    const confirmPasswordField = document.getElementById('nhap_lai_mat_khau');
    ...
    const isUpdateMode = !!document.getElementById('nguoi_dung_id').value;
    const successMessage = isUpdateMode ? 'Cập nhật người dùng thành công!' : 'Tạo người dùng thành công!';
    const loadingMessage = isUpdateMode ? 'Đang cập nhật người dùng...' : 'Đang tạo người dùng...';
...
/**
 * Tải dữ liệu người dùng nếu ở chế độ cập nhật
 */
const initUserDataLoad = () => {
  // Lấy ID người dùng từ URL nếu có
  const urlParams = new URLSearchParams(window.location.search);
  const userId = urlParams.get('id');
  
  // Thêm field ẩn để lưu ID người dùng
  ...
  // Nếu có ID, tải dữ liệu người dùng
  if (userId) {
    loadUserData(userId);
    
    // Cập nhật tiêu đề và nút submit
    document.querySelector('h1').textContent = 'Cập nhật thông tin người dùng';
    document.querySelector('button[type="submit"]').textContent = 'Cập nhật';
    
    // Ẩn trường mật khẩu nếu là cập nhật
    const passwordSection = document.getElementById('password-section');
    if (passwordSection) {
      const passwordToggle = document.createElement('div');
      passwordToggle.className = 'flex items-center mb-4';
      passwordToggle.innerHTML = `
        <input type="checkbox" id="change_password" class="mr-2">
        <label for="change_password">Thay đổi mật khẩu</label>
      `;
      passwordSection.prepend(passwordToggle);
